...
export const getFollowingList = async (nickname, page = 0) => {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/api/follows/${nickname}/followings`,
      {
        headers: {
...
export const getFollowerList = async (nickname, page = 0) => {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/api/follows/${nickname}/followers`,
      {
        headers: {
...
export const follow = async (nickname) => {
  ...
  try {
    ...
    if (response.data.code === "1O00") {
      //팔로우 성공 코드 "1O00"
      console.log("팔로우 추가 성공:", response);
    ...
    return {
      success: false,
      error: error.response ? error.response.data : "팔로우 추가 실패",
    };
  }
};
...
export const unfollow = async (followId) => {
  ...
  try {
    ...
    return {
      success: false,
      error: error.response ? error.response.data : "팔로우 취소 실패",
    };
  }
};
