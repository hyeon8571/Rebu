...
export const getCommonMyProfile = async () => {
  ...
  try {
    ...
    if (response.data.code === "1C10") { //일반 프로필 마이페이지 조회 성공 코드
...
export const getEmployeeMyProfile = async () => {
  const access = localStorage.getItem("access");
  try {
    const response = await axios.get(`${BASE_URL}/api/profiles/employees/mypage`, {
      // const response = await axios.get(`${BASE_URL}/api/profiles/${nickname}`, {
      headers: {
        "access": access,
        "Content-Type": "application/json",
    ...
    if (response.data.code === "1C10") { //직원 프로필 마이페이지 조회 성공 코드
      console.log("getEmployeeMyProfile 성공", response)
      return { success: true, data: response.data.body };

    } else if (response.data.code === "0A15") { //엑세스 토큰 만료 코드
...
export const getShopMyProfile = async () => {
  ...
  try {
    ...
    if (response.data.code === "1C10") { //매장 프로필 마이페이지 조회 성공 코드
...
export const getCommonProfileInfo = async () => {
  ...
  try {
    ...
    if (response.data.code === "1C11") { // 성공시
    ...
    return { success: false, error: "An error occurred while fetching the profile info." };
  }
};
...
export const updateProfileImage = async (nickname, imageFile) => {
  ...
  try {
    ...
    // 요청이 성공했을 때의 처리
    if (response.data.code === "1C05") { // 1C05은 프로필 이미지 변경 성공 코드 (가정)
    ...
    return { success: false, error: error.response?.data || "프로필 이미지 변경에 실패했습니다." };
  }
};